...
void create(string *chaine_dest, char *chaine_src){
    ...
    chaine_dest->str = malloc(sizeof(chaine_src));
    chaine_dest->str = chaine_src;
...
void add(string *chaine, char c){
    int TAILLE = strlen(chaine->str);
    char* nouveau = realloc(chaine->str, (TAILLE+1)*sizeof(char));
    if (nouveau) {
        chaine->str = nouveau;
    }
    chaine->str[TAILLE]=c;
