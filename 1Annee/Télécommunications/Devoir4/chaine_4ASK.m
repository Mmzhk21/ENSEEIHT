% 4    Comparaison de modulations sur frequence porteuse
...
% Calcul TES
TES_ASK = length(find(Symboles ~= symboles_decides)) / length(Symboles);
...
%     ylabel('Q');
    ...
    % Calcul TES
    TES_ASK(i + 1) = length(find(Symboles ~= symboles_decides)) / length(Symboles);
    
    % Taux d'erreurs binaires
    TEB_ASK(i + 1) = TES_ASK(i + 1) / 2;
end
    % Les constellations en sortie du mapping et de l’échantillonneur
    figure;
    plot(real(z_echant), 0, 'r*');
...
% et le TEB théorique
figure;
semilogy([0 : 8], TEB_ASK, 'r*');
hold on
semilogy([0 : 8], (3 / 4) * qfunc(sqrt((4 / 5) * 10 .^ ([0 : 8] / 10))));
grid
