...
% Calcul du TEB
TEB_PSK = length(find(bits_decides ~= bits)) / length(bits)
...
for i = 0 : 8
    ...
    % Calcul du TES
    TES_PSK(i + 1) = length(find(bits_decides ~= bits)) / length(bits);
    
    % Calcul du TEB
    TEB_PSK(i + 1) = TES_PSK(i + 1) / log2(M);
end
...
% et le TEB théorique
figure;
semilogy([0 : 8], TEB_PSK, 'r*');
hold on
semilogy([0 : 8], (2 / log2(M)) * qfunc(sqrt(2 * log2(M) * 10 .^ ([0 : 8] / 10)) * sin(pi / M)));
grid
