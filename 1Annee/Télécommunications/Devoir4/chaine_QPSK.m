...
% Calcul du TES
TES_QPSK = length(find(symboles_decides ~= dk)) / (2 * length(dk));

% Calcul du TEB
TEB_QPSK = TES_QPSK / log2(4)
...
%     ylabel('Q');
    ...
    % Calcul du TES
    TES_QPSK(i + 1) = length(find(symboles_decides ~= dk)) / (length(dk));
    
    % Calcul du TEB
    TEB_QPSK(i + 1) = TES_QPSK(i + 1) / 2;
end
